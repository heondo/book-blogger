...
router.get('/:id', (req, res, next) => {
  ...
  db.query(singleReviewQuery, [reviewID, reviewID, reviewID], (err, data) => {
    ...
    data[0].review_likes = data[0].review_likes ? data[0].review_likes.split(',').map(id => parseInt(id)) : [];
...
router.get('/', (req, res, next) => {
  ...
  db.query(getReviewsListQuery, (err, data) => {
    ...
    data.forEach(review => {
      ...
      review.review_likes = review.review_likes ? review.review_likes.split(',').map(id => parseInt(id)) : [];
